 ...
 *      that resolves — via a `GET` — to the SAME room; both `setVenue` writes stamp identical
 *      venue columns, and `venue_provisioned_at` differs only by the race's gap (last write
 *      wins, harmless). That is why this ticket writes no `ON CONFLICT` clause anywhere and
 ...
 *      owners: the venue repair job (`apps/api/src/jobs/meeting-venue-repair.ts`) re-derives
 *      this name and adopts the room, and cancel-time deletion lives in
 *      `meeting-availability.ts`'s `tearDownRoomBestEffort`. Rooms are
...
/** The room-name prefix, shared with the SQL twin (`meetings.ts`'s `meetingVenueReadySql`). */
export const DAILY_ROOM_NAME_PREFIX = 'balo-';
...
export function dailyRoomNameForMeeting(meetingId: string): string {
  return `${DAILY_ROOM_NAME_PREFIX}${meetingId.replace(/-/g, '').toLowerCase()}`;
}
