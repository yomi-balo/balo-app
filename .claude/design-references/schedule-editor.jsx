...
const Icons = {
  ...
  search: (p) => <svg width={p.size||16} height={p.size||16} viewBox="0 0 24 24" fill="none" stroke={p.color||'currentColor'} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={p.style}><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>,
...
function Toggle({ checked, onChange }) {
  return (
    <div onClick={() => onChange(!checked)} style={{ width: 36, height: 20, borderRadius: 10,
      background: checked ? c.primary : c.border, position: 'relative', cursor: 'pointer',
    ...
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      <button onClick={() => setOpen(!open)}
        style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px',
          borderRadius: 7, border: `1px solid ${open ? c.primary : c.border}`,
          background: open ? c.primaryLight : c.surface, cursor: 'pointer',
          fontSize: 13, fontWeight: 500, color: c.text, whiteSpace: 'nowrap',
          boxShadow: open ? `0 0 0 3px ${c.primaryGlow}` : 'none',
          transition: 'all 0.15s', minWidth: 98 }}>
        <Icons.clock size={12} color={open ? c.primary : c.textTertiary} />
        {selected?.label || '—'}
        <Icons.chevDown size={11} color={open ? c.primary : c.textTertiary}
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, zIndex: 30,
          ...
          minWidth: 130, animation: 'dropIn 0.15s ease-out' }}>
          {options.map(opt => (
            <div key={opt.value} onClick={() => { onChange(opt.value); setOpen(false); }}
              ...
              onMouseEnter={e => { if (value !== opt.value) e.target.style.background = c.surfaceSubtle; }}
...
function DayRow({ day, ranges, onToggle, onUpdateRange, onAddRange, onRemoveRange, enabled }) {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '10px 0',
        borderBottom: `1px solid ${c.borderSubtle}` }}>
      {/* Day name + toggle */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: 110, flexShrink: 0, paddingTop: 4 }}>
        <Toggle checked={enabled} onChange={onToggle} />
        <span style={{ fontSize: 13, fontWeight: enabled ? 600 : 450,
          color: enabled ? c.text : c.textTertiary, transition: 'all 0.2s', width: 32 }}>
          {day.label}
        </span>
      </div>
      ...
      <div style={{ flex: 1 }}>
        {!enabled ? (
          <div style={{ paddingTop: 4 }}>
            <span style={{ fontSize: 13, color: c.textTertiary, fontStyle: 'italic' }}>Unavailable</span>
          </div>
        ) : (
          <div>
            {ranges.map((range, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8,
                ...
                <TimeSelect value={range.end} onChange={(v) => onUpdateRange(i, 'end', v)} min={range.start} />
                {ranges.length > 1 && (
                  <button onClick={() => onRemoveRange(i)}
                    style={{ padding: 4, borderRadius: 6, background: 'none', border: 'none',
                      cursor: 'pointer', display: 'flex', alignItems: 'center',
                      color: c.textTertiary, opacity: hover ? 1 : 0, transition: 'opacity 0.15s' }}>
            ...
            {ranges.length < 3 && (
              <button onClick={onAddRange}
                style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 7,
                  ...
                  color: hover ? c.primary : c.textTertiary, transition: 'all 0.2s' }}>
...
function TimezoneSelector({ value, onChange }) {
  ...
  useEffect(() => {
    const update = () => {
      try {
        const time = new Date().toLocaleTimeString('en-AU', { timeZone: value, hour: '2-digit', minute: '2-digit', hour12: true });
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)}
        style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%',
          ...
          boxShadow: open ? `0 0 0 3px ${c.primaryGlow}` : 'none', transition: 'all 0.2s' }}>
        ...
        <span style={{ fontSize: 11, color: c.textTertiary, fontFamily: 'monospace', marginRight: 4 }}>
          {selected.offset}
        </span>
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 40,
          ...
          <div style={{ padding: '8px 8px 4px', borderBottom: `1px solid ${c.borderSubtle}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px',
              borderRadius: 7, background: c.surfaceSubtle, border: `1px solid ${c.borderSubtle}` }}>
              <Icons.search size={13} color={c.textTertiary} />
              <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)}
                placeholder='Search timezone...'
                style={{ flex: 1, fontSize: 13, color: c.text, border: 'none', outline: 'none',
                  background: 'transparent' }} />
            </div>
          </div>
          <div style={{ maxHeight: 240, overflowY: 'auto' }}>
            {filtered.map(tz => (
              <div key={tz.value} onClick={() => { onChange(tz.value); setOpen(false); setQuery(''); }}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 14px',
                  cursor: 'pointer', background: value === tz.value ? c.primaryLight : 'transparent',
                  transition: 'background 0.1s' }}
                onMouseEnter={e => { if (value !== tz.value) e.currentTarget.style.background = c.surfaceSubtle; }}
                onMouseLeave={e => { if (value !== tz.value) e.currentTarget.style.background = 'transparent'; }}>
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: 13, fontWeight: value === tz.value ? 600 : 400,
                    color: value === tz.value ? c.primary : c.text }}>{tz.label}</span>
                  <span style={{ fontSize: 11, color: c.textTertiary, marginLeft: 6 }}>{tz.value.split('/')[1]?.replace('_', ' ')}</span>
                </div>
                <span style={{ fontSize: 11, color: c.textTertiary, fontFamily: 'monospace' }}>{tz.offset}</span>
...
function ScheduleSavedPreview({ schedule, timezone }) {
  ...
  return (
    <div style={{ padding: '16px', borderRadius: 10, background: c.successLight,
      border: `1px solid ${c.successBorder}`, animation: 'fadeIn 0.3s ease-out' }}>
      ...
      {tz && (
        <p style={{ fontSize: 12, color: c.success, margin: '8px 0 0', display: 'flex',
...
export default function ScheduleEditor() {
  ...
  const totalHours = DAYS.reduce((sum, d) => {
    return sum + (schedule[d.key] || []).reduce((s, r) => {
      const [sh, sm] = r.start.split(':').map(Number);
      const [eh, em] = r.end.split(':').map(Number);
    ...
    <div style={{ minHeight: '100vh', background: c.bg,
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
        <div style={{ display: 'inline-flex', gap: 4, padding: 4, borderRadius: 10,
          background: '#F1F4F8', border: '1px solid #EAEFF5' }}>
          {DEMO_STATES.map(s => (
            <button key={s.key} onClick={() => setDemoState(s.key)}
              style={{ padding: '6px 16px', borderRadius: 7, fontSize: 12, fontWeight: 550,
      ...
      <div style={{ maxWidth: 660, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 28, ...slideUp(0) }}>
          ...
          <p style={{ fontSize: 14, color: c.textSecondary, margin: 0, lineHeight: 1.6 }}>
            Set the days and times you're available for consultations. Clients will only be
            able to book within these windows.
          </p>
        </div>
        ...
        {isEmpty && (
          <div style={{ ...slideUp(0.05) }}>
            <div style={{ padding: '56px 40px', textAlign: 'center', background: c.surface,
              borderRadius: 16, border: `1px solid ${c.border}`,
              boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ width: 56, height: 56, borderRadius: 15, margin: '0 auto 18px',
                background: 'rgba(8,145,178,0.08)', border: '1px solid rgba(8,145,178,0.15)',
                display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: c.textSecondary, margin: '0 auto 24px',
                lineHeight: 1.6, maxWidth: 360 }}>
                Tell clients when you're available for consultations. You can always update
                this later and add date-specific blocks for holidays or leave.
              </p>
              <button onClick={() => setDemoState('editing')}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px',
                  borderRadius: 10, fontSize: 13, fontWeight: 650,
        ...
        {(isEditing || isSaved) && (
          <div key={demoState}>
            ...
            <div style={{ background: c.surface, borderRadius: 16, border: `1px solid ${c.border}`,
              boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '20px 22px',
              marginBottom: 12, ...slideUp(0.04) }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
                <Icons.globe size={13} color={c.primary} />
                <span style={{ fontSize: 11, fontWeight: 700, color: c.primary,
                  textTransform: 'uppercase', letterSpacing: '0.07em' }}>Timezone</span>
              </div>
              <TimezoneSelector value={timezone} onChange={setTimezone} />
              <p style={{ fontSize: 12, color: c.textTertiary, margin: '8px 0 0' }}>
                Your schedule times are set in this timezone. Clients see slots converted to their local time.
              </p>
            </div>
            ...
            <div style={{ background: c.surface, borderRadius: 16, border: `1px solid ${c.border}`,
              boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '20px 22px',
              marginBottom: 12, ...slideUp(0.07) }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                marginBottom: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <Icons.calendar size={13} color={c.primary} />
                  <span style={{ fontSize: 11, fontWeight: 700, color: c.primary,
                    textTransform: 'uppercase', letterSpacing: '0.07em' }}>Weekly Hours</span>
                </div>
                {/* Total hours summary */}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 6 }}>
                <div style={{ width: 110, flexShrink: 0 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, color: c.textTertiary,
                    textTransform: 'uppercase', letterSpacing: '0.05em' }}>Day</span>
                </div>
                <span style={{ fontSize: 10, fontWeight: 600, color: c.textTertiary,
                  textTransform: 'uppercase', letterSpacing: '0.05em' }}>Available hours</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <button onClick={() => setSchedule({ ...DEFAULT_SCHEDULE })}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px',
                    borderRadius: 7, fontSize: 12, fontWeight: 500, background: c.surface,
                    border: `1px solid ${c.border}`, cursor: 'pointer', color: c.textSecondary }}>
                ...
                <button onClick={() => {
                  ...
                  setSchedule(prev => ({
                    ...prev,
                    tuesday: [...monRanges], wednesday: [...monRanges],
                    thursday: [...monRanges], friday: [...monRanges],
                  }));
                }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px',
                  borderRadius: 7, fontSize: 12, fontWeight: 500, background: c.surface,
                  border: `1px solid ${c.border}`, cursor: 'pointer', color: c.textSecondary }}>
            ...
            {/* Save CTA */}
            {!isSaved && (
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, ...slideUp(0.1) }}>
                <button style={{ padding: '10px 18px', borderRadius: 10, fontSize: 13,
                  fontWeight: 550, background: c.surface, border: `1px solid ${c.border}`,
                  cursor: 'pointer', color: c.textSecondary }}>
                  Cancel
                </button>
                <button onClick={() => setDemoState('saved')}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', ...slideUp(0.12) }}>
                <button onClick={() => setDemoState('editing')}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px',
                    borderRadius: 9, fontSize: 13, fontWeight: 550, background: c.surface,
                    border: `1px solid ${c.border}`, cursor: 'pointer', color: c.textSecondary }}>
                  Edit schedule
                </button>
              </div>
