...
const FileText = ({ size = 12, color = c.faint }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color}
    ...
    <path d="M14 2v4a2 2 0 0 0 2 2h4" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" />
  </svg>
);
...
const P = {
  ...
  milestones: [
    ['Discovery & solution design', '$3,125', 'Stakeholder workshops, current-state review, and a documented target CPQ architecture.'],
    ['CPQ configuration & pricing rules', '$6,250', 'Product bundles, discount schedules, approval flows, and quote templates built and configured.'],
    ['Data migration & validation', '$3,125', 'Migration of existing product and pricing data, with reconciliation against source records.'],
    ['UAT support & team training', '$3,125', 'Guided user-acceptance testing plus two enablement sessions for the sales team.'],
  ],
  installments: [
  ...
  exclusions: 'Third-party license costs and ongoing managed support beyond the training period are not included.',
...
const Label = ({ children, style }) => (
  <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: '0.08em', color: c.muted, ...style }}>{children}</div>
);

export default function ProposalPdfReference() {
  return (
    <div style={{ minHeight: '100vh', background: '#EEF1F5', padding: '32px 16px', fontFamily: "'Geist', system-ui, sans-serif", display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ maxWidth: 620, width: '100%', fontSize: 12, color: c.muted, lineHeight: 1.5 }}>
        ...
        <code style={{ fontSize: 11 }}>apps/web/src/lib/project-request/proposal/pdf/proposal-pdf-document.tsx</code>. Editing this file changes nothing that ships.
      </div>

      {/* A4 page — 210:297 ratio */}
      <div style={{
        width: 620, aspectRatio: '210 / 297', background: c.page, color: c.ink,
        boxShadow: '0 8px 28px rgba(15,23,42,0.14)', display: 'flex', flexDirection: 'column', overflow: 'hidden',
      }}>
        <div style={{ padding: '30px 40px 0', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <span style={{ fontSize: 18, fontWeight: 700, color: c.brand, letterSpacing: '-0.4px' }}>Balo</span>
            <span style={{
              fontSize: 9, fontWeight: 600, color: c.brand, background: c.brandSoft,
              border: `1px solid ${c.brandBorder}`, borderRadius: 999, padding: '3px 9px',
            }}>{P.pricingMethod}</span>
          </div>
          ...
          <div style={{ marginTop: 16, border: `1px solid ${c.border}`, borderRadius: 10, overflow: 'hidden' }}>
            <div style={{
              ...
              padding: '14px 16px', background: c.brandSoft, borderBottom: `1px solid ${c.brandBorder}`,
            }}>
              <Label style={{ color: c.muted }}>TOTAL AMOUNT</Label>
              <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.01em', lineHeight: 1 }}>{P.total}</div>
            </div>
            <div style={{ display: 'flex' }}>
              {[
                ...
                <div key={label} style={{ flex: 1, padding: '10px 14px', borderRight: i < 3 ? `1px solid ${c.hair}` : 'none' }}>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ fontSize: 9.5, lineHeight: 1.6, color: '#334155', marginTop: 6 }}>{P.overview}</div>
          </div>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 7 }}>
              {P.milestones.map(([title, value, desc], i) => (
                <div key={title} style={{ border: `1px solid ${c.border}`, borderRadius: 8, padding: 11 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, flex: 1 }}>{i + 1}. {title}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, whiteSpace: 'nowrap' }}>{value}</span>
                  </div>
                  <div style={{ fontSize: 9, color: c.muted, lineHeight: 1.5, marginTop: 4 }}>{desc}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ marginTop: 6, border: `1px solid ${c.border}`, borderRadius: 8, padding: 12 }}>
              {P.installments.map(([label, pct, amount]) => (
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
                  <span style={{ fontSize: 10, fontWeight: 600 }}>{label} — {amount}</span>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ fontSize: 9.5, lineHeight: 1.6, color: '#334155', marginTop: 6 }}>{P.exclusions}</div>
          </div>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {P.attachments.map((name) => (
                <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 7, border: `1px solid ${c.border}`, borderRadius: 8, padding: '8px 11px' }}>
                  <FileText />
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div style={{ marginTop: 6, border: `1px solid ${c.border}`, background: c.subtleBg, borderRadius: 8, padding: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 600, marginBottom: 5 }}>Balo standard terms apply</div>
              {P.standardTerms.map((term) => (
                <div key={term} style={{ display: 'flex', gap: 6, marginBottom: 3 }}>
                  <span style={{ fontSize: 9, color: c.muted }}>•</span>
                  <span style={{ flex: 1, fontSize: 9, color: c.muted, lineHeight: 1.5 }}>{term}</span>
                </div>
              ))}
            </div>
          </div>
