  ...
  if (status === 'error')
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        {lastSynced && (
          <span style={{ fontSize: 12, color: c.textTertiary }}>· last synced {lastSynced}</span>
        )}
      </div>
    );
  return null;
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 7 }}>
        <span style={{ fontSize: 13, color: c.text, fontWeight: calendar.primary ? 600 : 400 }}>
          {calendar.name}
        </span>
        {calendar.primary && (
          <span
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        {calendar.primary && (
          <span style={{ fontSize: 11, color: c.textTertiary, fontStyle: 'italic' }}>
            Always on
          </span>
        )}
        <Toggle
    ...
    <Card style={{ overflow: 'hidden', ...slideUp(0.04) }}>
      ...
      {isError && (
        ...
        >
          ...
          <p style={{ fontSize: 13, color: c.error, margin: 0, lineHeight: 1.5 }}>
            {connection.errorMsg}
          </p>
        </div>
      )}
    ...
    <Card style={{ overflow: 'hidden', animation: 'dropIn 0.2s ease-out' }}>
      ...
      <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <ProviderButton
          ...
          alreadyConnected={googleConnected}
        />
        <ProviderButton
          provider="microsoft"
          onClick={() => onConnect('microsoft')}
          alreadyConnected={microsoftConnected}
        />
      </div>
    </Card>
  );
      ...
      <div style={{ maxWidth: 620, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 32, ...slideUp(0) }}>
          ...
          <p style={{ fontSize: 14, color: c.textSecondary, margin: 0, lineHeight: 1.6 }}>
            Connect your calendar so Balo only shows clients times when you're genuinely free. Your
            event details are never shared.
          </p>
        </div>
        ...
        {viewState === 'empty' && (
          <Card style={{ padding: '24px 24px 20px', ...slideUp(0.05) }}>
            ...
            <p
              style={{ fontSize: 13, color: c.textSecondary, margin: '0 0 18px', lineHeight: 1.6 }}
            >
              Balo reads your calendar events to calculate your real availability. Clients will only
              see open slots, never your event titles or details.
            </p>
