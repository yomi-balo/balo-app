...
| `findConnectionsByEndUserAccountId`                            | **Shipped, LIVE — two callers.** `packages/db/src/repositories/calendar.ts`; BAL-575 gave it a required `{ excludingConnectionId }` option and two production callers — the OAuth callback's refusal-cleanup (`discardRefusedEndUserAccount`, passing `null`) and `disconnectProvider`'s shared-account guard (passing its own row id). Both `routes/calendar/webhook.ts` and `jobs/calendar-subscription-reconcile.ts` resolve webhook identity by primary key through `findConnectionById` instead — this array reader is never an identity resolver; never take `[0]` from it | BAL-467, callers = BAL-575                         |
...
connecting the same Google account is possible (inferred, not captured — BAL-577 checks it)). You
would have to fan out.
...
| Header table: `callApiroc`, `end_user_account_id` "INERT" / "written by nobody"                                            | Both are live as of BAL-396 — the OAuth connect callback writes `end_user_account_id` and calls `callApiroc` for real, `freeBusy.get` and the health probe do too. Only the consultation-event write/delete/reconcile trio and `findConnectionsByEndUserAccountId` remain INERT. ⚠ BAL-575 (2026-09) — PARTIALLY SUPERSEDED: `findConnectionsByEndUserAccountId` now has two live callers (see the header row)                                                                          |
