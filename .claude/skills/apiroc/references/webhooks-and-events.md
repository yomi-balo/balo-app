...
| Apiroc **event writes** (create/delete/reconcile/UPDATE)       | **Shipped, complete, tested — but the create/delete/reconcile trio is still INERT.** `apps/api/src/services/consultation-events/` now also ships `update-consultation-event.ts` (BAL-409), which IS LIVE — called from the `meeting-calendar-amend` BullMQ job | BAL-396, caller = BAL-400; update caller = BAL-409 |
