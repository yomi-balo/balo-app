 ...
 * brand-new signup has none, and the pre-submit check has no userId.
 *
 * Single source of truth: `runDomainJoin` maps this reason → its granular outcome;
 * the pre-submit domain check derives the show/hide boolean from it. The two can
 ...
 * returns `'personal_owner'` for every company match ⇒ the pre-submit check
 * returns `new` (never `matched`) ⇒ the compulsory field is always shown for
 * corporate domains. `matched` is dormant, exactly like the inert join engine, and
 * BOTH light up together when the shared-org creation seam ships — because they
...
/**
 * Convenience boolean for the pre-submit domain check: true only when the join
 * flow would own the company identity (engine acts) ⇒ HIDE the compulsory field.
 * A personal-owner / directory / mode-off match ⇒ false ⇒ SHOW the field.
 */
