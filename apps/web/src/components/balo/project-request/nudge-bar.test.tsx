...
import { NUDGE_ICONS } from './nudge-icons';
...
describe('NudgeBar', () => {
  ...
  /**
   * ⚠⚠ RSC SERIALIZABILITY. `nudgeFor()` runs in a Server Component and its `primary`/`secondary`
   * descriptors are handed to `NudgeActions`, which is `'use client'`. Carrying a Lucide icon —
   * a `forwardRef` object, `{$$typeof, render: fn}` — put a function across that boundary and
   * killed the request-detail page outright:
   *
   *   Only plain objects can be passed to Client Components from Server Components.
   *   Functions cannot be passed directly to Client Components ... {$$typeof: ..., render: function Users}
   *
   * The `NudgeIconName` type forbids it, but a `as` cast or a brand-new nudge table would slip
   * past `tsc`. This walks EVERY cell of the real tables and asserts the whole object survives a
   * JSON round-trip — the property the RSC boundary actually requires.
   */
  describe('nudge content is RSC-serializable', () => {
    const LENSES = ['client', 'expert', 'admin'] as const;
    const STATUSES = [
      'draft',
      'requested',
    ...
    const every = LENSES.flatMap((lens) =>
      STATUSES.flatMap((status) =>
        REL_STATUSES.map((rel) => ({ lens, status, rel, nudge: nudgeFor(lens, status, rel) }))
      )
    );

    it('covers a real, non-trivial number of cells (guards against a vacuous walk)', () => {
      expect(every.length).toBeGreaterThan(100);
      expect(every.filter((c) => c.nudge !== null).length).toBeGreaterThan(10);
    });

    it.each(['client', 'expert', 'admin'] as const)(
      'every %s nudge survives a JSON round-trip, icons included',
      (lens) => {
        for (const cell of every.filter((c) => c.lens === lens && c.nudge !== null)) {
          const nudge = cell.nudge!;
          const where = `${cell.lens}:${cell.status}:${String(cell.rel)}`;
          // A function anywhere in here is what broke the page.
          expect(JSON.parse(JSON.stringify(nudge)), where).toEqual(nudge);
          expect(typeof nudge.icon, where).toBe('string');
          if (nudge.primary) expect(typeof nudge.primary.icon, where).toBe('string');
          if (nudge.secondary) expect(typeof nudge.secondary.icon, where).toBe('string');
        }
      }
    );

    it('EXPERT_GATED_NUDGE is serializable too (it bypasses nudgeFor)', () => {
      expect(JSON.parse(JSON.stringify(EXPERT_GATED_NUDGE))).toEqual(EXPERT_GATED_NUDGE);
      expect(typeof EXPERT_GATED_NUDGE.icon).toBe('string');
    });

    it('every icon token resolves to a real component', () => {
      for (const cell of every) {
        if (!cell.nudge) continue;
        expect(NUDGE_ICONS[cell.nudge.icon]).toBeDefined();
        if (cell.nudge.primary) expect(NUDGE_ICONS[cell.nudge.primary.icon]).toBeDefined();
        if (cell.nudge.secondary) expect(NUDGE_ICONS[cell.nudge.secondary.icon]).toBeDefined();
      }
    });
  });

  describe('NudgeBar — done variant', () => {
