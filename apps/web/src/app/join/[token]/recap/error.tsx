...
/**
 * Route-segment error boundary for the guest recap INDEX (CLAUDE.md requires one per new
 * segment). Generic and leak-free — cloned verbatim in posture from `[meetingId]/error.tsx`.
 *
 ...
 *
 * ⚠ NO SIGN-IN CTA, for the same reason `[token]/error.tsx` and `[meetingId]/error.tsx` have
 * none: a guest has no account. "Try again" is the only recovery that is real for this audience.
 */
export default function GuestRecapIndexError({
  reset,
  ...
  return (
    <div className="border-border bg-card mx-auto w-full max-w-md rounded-2xl border p-8 text-center shadow-sm">
      ...
      <p className="text-muted-foreground mt-2 text-[13px] leading-relaxed">
        We couldn&apos;t open your recaps just now. Please try again in a moment.
      </p>
      <button
        type="button"
        onClick={reset}
