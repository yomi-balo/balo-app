    ...
    <>
            ...
            return (
              <div key={step.key} className="relative z-10 flex flex-col items-center">
                <motion.button
                  ...
                  aria-label={`Step ${i + 1}: ${step.label}${status === 'completed' ? ' (completed)' : status === 'skipped' ? ' (skipped)' : ''}`}
      ...
      <div className="mb-4 md:hidden">
        ...
        <p className="text-muted-foreground mt-2 text-center text-xs">
          Step {currentStep + 1} of {STEP_CONFIG.length} &mdash; {STEP_CONFIG[currentStep]!.label}
        </p>
      </div>
    </>
  );
