 ...
 *
 * BAL-516 — `UNPROVISIONED_WALLET` / `projectSavedCard` / `projectWalletSnapshot` moved to
 * `@/lib/credit/wallet-read` (was a local, unexported shape here) so `/settings/billing`'s new
 * read shares one definition rather than a second copy of "is this card chargeable".
 */
...
export default async function TopUpPage() {
  ...
  const snapshot: WalletSnapshot =
    wallet === undefined ? UNPROVISIONED_WALLET : projectWalletSnapshot(wallet);
