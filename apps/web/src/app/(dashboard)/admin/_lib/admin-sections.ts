...
import { Inbox, Layers, ScanSearch, UserPlus, Activity } from 'lucide-react';
 ...
 *
 * ⚠ FIVE CHIPS, AND THE SET IS "ROUTES NESTED UNDER `(dashboard)/admin`" — NOT "the Balo admin
 ...
 * own `applications` chip — and BAL-550 adds `health` (`/admin/health/capture`), so the rule and
 * the list finally agree.
 */
export interface AdminSectionMeta {
  readonly key: 'home' | 'applications' | 'lookup' | 'catalogue' | 'health';
