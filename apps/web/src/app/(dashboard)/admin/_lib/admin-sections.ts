...
import { Inbox, Layers, ScanSearch, ShieldCheck, UserPlus, Activity } from 'lucide-react';
 ...
 *
 * ⚠ SIX CHIPS, AND THE SET IS "ROUTES NESTED UNDER `(dashboard)/admin`" — NOT "the Balo admin
 ...
 * BAL-551 shipped `/admin/lookup` WITHOUT a chip — a one-line gap BAL-549 closed alongside its
 * own `applications` chip — BAL-550 added `health` (`/admin/health/capture`), and BAL-561 added
 * `staff-access` (`/admin/staff-access`), so the rule and the list finally agree.
 */
export interface AdminSectionMeta {
  readonly key: 'home' | 'applications' | 'lookup' | 'catalogue' | 'health' | 'staff-access';
...
export const ADMIN_SECTION_ORDER: readonly AdminSectionMeta[] = [
  ...
  { key: 'health', label: 'Capture health', href: '/admin/health/capture', icon: Activity },
  {
    key: 'staff-access',
    label: 'Staff access',
 ...
 * `'applications'` (segment index 2 only — the detail route's id is never inspected), and by the
 * same rule `/admin/health/capture` → `'health'`; anything else (including `/admin/__proto__`,
 * `/settings/admin`) → `null`. Segment index 2 tested
