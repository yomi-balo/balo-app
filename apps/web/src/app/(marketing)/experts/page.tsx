...
export default async function ExpertsPage({
  ...
  const frame = (content: React.ReactNode): React.JSX.Element => (
    <div className="bg-background min-h-screen px-4 py-7 sm:px-6 md:px-8 md:py-8">
      <div className="mx-auto max-w-[1320px]">
        <SearchHeroPlaceholder initialQuery={filters.q} />
        {content}
      </div>
    </div>
  );
  ...
  let response;
  try {
    response = await searchExperts(filters);
  } catch (error) {
  ...
  return frame(
    <div className="flex items-start gap-8">
      ...
      <aside className="hidden w-[252px] shrink-0 md:block">
        <PlaceholderFilterRail facetCounts={facetCounts} filters={filters} />
      </aside>
        ...
        {isPageBeyondRange && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground mt-1 max-w-sm text-sm leading-relaxed">
              There are {total} experts matching your filters, but this page is past the end of the
              results.
            </p>
            <Link
