...
describe('POST /webhooks/stripe', () => {
  ...
  it('returns 500 (never 200) when the transaction resolved with NO committed marker', async () => {
    ...
    // ⚠ A NON-SENTRY OBSERVABLE FOR THE SAME DISTINCTION, so the pin survives someone deciding
    // the Sentry assertion below is incidental. `findByEventId` runs TWICE only when the
    // transaction resolved and the POST-COMMIT read-back then ran: once on the pre-transaction
    // replay short-circuit, once on the base `db` afterwards. Delete the read-back and this
    // count drops to 1 while the 500 (and every other assertion here) stays green.
    expect(mockFindByEventId).toHaveBeenCalledTimes(2);
  ...
  it('returns 500 when markProcessed stamps ZERO rows (the in-transaction row-count check)', async () => {
    ...
    expect(res.statusCode).toBe(500);
    // ⚠ A NON-SENTRY OBSERVABLE FOR THE SAME DISTINCTION — the counterpart of the `2` asserted in
    // the post-commit test above. The in-transaction throw aborts BEFORE the transaction resolves,
    // so the post-commit read-back never runs and `findByEventId` is called exactly ONCE (the
    // pre-transaction replay short-circuit). Delete the row-count guard and this input still 500s
    // — via the read-back — but the count becomes 2. Both guards therefore stay independently
    // pinned even if the Sentry-stage assertions are ever simplified away.
    expect(mockFindByEventId).toHaveBeenCalledTimes(1);
